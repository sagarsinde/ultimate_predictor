...
from hybrid_1.features import load_raw_data, build_features, slice_window, get_window_size
from hybrid_1.validator import load_state, _train_single_model, _predict_single
...
def evaluate_month(market, target_month='2026-06'):
    ...
    for pred_idx in june_indices:
        ...
        pred_date_obj = pd.to_datetime(df.iloc[pred_idx]['Date'])
        current_dow = pred_date_obj.dayofweek
        
        m_probs, e_probs = _predict_single(model_m, model_e, model_type, X_pred, last_m, last_e, current_dow)
