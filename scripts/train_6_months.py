"""
Train all standardized fast models on the last 6 months of data.
Saves models to trained_models/6_months/<market>/
"""
...
def train_6_months(market):
    print(f"\n--- Training 6-Month Models for {market.upper()} ---")
    
    df_raw = load_raw_data(market)
    # Get 6 months of draws (approx 2x 3m)
    window_draws = get_window_size(market, '3m') * 2
    df = slice_window(df_raw, window_draws)
    
    print(f"Loaded {len(df)} draws (approx 6 months). Building features...")
    ...
    dir_path = os.path.join("trained_models", "6_months", market)
